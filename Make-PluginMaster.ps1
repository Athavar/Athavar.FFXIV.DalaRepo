...
$dlTemplateUpdate = "https://raw.githubusercontent.com/Athavar/Athavar.FFXIV.DalaRepo/master/{0}/{1}/latest.zip"
...
Foreach-Object {
    ...
    if (($output | Where-Object {$_.InternalName -eq $content.InternalName}).Count -eq 0)
    {
        ...
        $installLink = $dlTemplateUpdate -f "testing", $internalName
        $content | add-member -Name "DownloadLinkInstall" $installLink -MemberType NoteProperty
